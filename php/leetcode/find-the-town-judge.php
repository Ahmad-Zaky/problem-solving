...
{
    // Validate Trust Length
    $judge = 1;
    $notJudges = [];
    $mayBeJudgesCount = [];
    $mayBeJudgesCount[$judge] = 0;
    foreach ( $trust as $trustPair) {
        $notJudges[$trustPair[0]] = '';

        // If Judge Trusts Any Body he is not a judge
        if (isset($notJudges[$judge])) {
            $judge = -1;
        }
        
        if (! isset($notJudges[$trustPair[1]])) {
            $judge = $trustPair[1];
        }

        if ($judge !== -1) {
            if ($trustPair[1] === $judge) {
                isset($mayBeJudgesCount[$judge]) 
                ? $mayBeJudgesCount[$judge] += 1
                : $mayBeJudgesCount[$judge] = 1;
            } 
        }
    }

    return isset($mayBeJudgesCount[$judge]) && $mayBeJudgesCount[$judge] === --$people 
        ? $judge
        : -1;
}
...
var_dump(
    testCases([
        ...
        assertEqual(
            // Output
            findJudge([[1,2],[1,3],[2,1],[2,3],[1,4],[4,3],[4,1]], 4), 
